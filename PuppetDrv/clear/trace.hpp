...
namespace trace
{
	...
	NTSTATUS SelfDeleteFile(PUNICODE_STRING path)
	{
	...
	BOOLEAN DeleteRegEditEntry(PUNICODE_STRING RegPath)
	{
		...
		//寻找内层
		RtlStringCbPrintfW((NTSTRSAFE_PWSTR)szPanth, 0x256, L"%ws\\Enum", RegPath->Buffer);
		RtlDeleteRegistryValue(RTL_REGISTRY_ABSOLUTE, (PCWSTR)szPanth, L"Count");
		RtlDeleteRegistryValue(RTL_REGISTRY_ABSOLUTE, (PCWSTR)szPanth, L"INITSTARTFAILED");
		RtlDeleteRegistryValue(RTL_REGISTRY_ABSOLUTE, (PCWSTR)szPanth, L"NextInstance");

		//删除内层
		RtlInitUnicodeString(&uPath, (PCWSTR)szPanth);
	...
	bool clear(const wchar_t* name, unsigned long stamp)
	{
		clear_cache(name, stamp);
		clear_unloaded_driver(name);
		clear_hash_bucket_list(name);
